...
class TestInterpretCommand(unittest.TestCase):
    def test_menu_keywords(self):
        self.assertEqual(cli.interpret_command("1")[0], "new")
        self.assertEqual(cli.interpret_command("n")[0], "new")
        self.assertEqual(cli.interpret_command("2")[0], "load")
        self.assertEqual(cli.interpret_command("q")[0], "quit")
        self.assertEqual(cli.interpret_command("  QUIT ")[0], "quit")
        self.assertEqual(cli.interpret_command("")[0], "empty")

    def test_pasted_token_is_detected(self):
        token = be.serialize(be.new_game(seed=3))
        kind, state = cli.interpret_command(token)
        self.assertEqual(kind, "token")
        self.assertEqual(state["version"], be.VERSION)

    def test_pasted_token_tolerates_whitespace(self):
        token = be.serialize(be.new_game(seed=3))
        kind, _ = cli.interpret_command("  " + token + "  ")
        self.assertEqual(kind, "token")

    def test_garbage_is_unknown(self):
        self.assertEqual(cli.interpret_command("hello there")[0], "unknown")
