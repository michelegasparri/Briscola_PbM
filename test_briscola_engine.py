...
class TestApplyMove(unittest.TestCase):
    ...
    def test_last_trick_recorded(self):
        s = be.new_game(seed=7)
        self.assertIsNone(s["last_trick"])
        lead = s["turn"]
        c1 = s["hands"][lead][0]
        s = be.apply_move(s, lead, c1)
        c2 = s["hands"][1 - lead][0]
        s = be.apply_move(s, 1 - lead, c2)
        lt = s["last_trick"]
        self.assertEqual(lt["plays"], [[lead, c1], [1 - lead, c2]])
        self.assertIn(lt["winner"], (0, 1))
        self.assertEqual(lt["points"], be.card_points(c1) + be.card_points(c2))
...
class TestPlayerView(unittest.TestCase):
    ...
    def test_last_trick_is_public(self):
        s = be.new_game(seed=42)
        self.assertIsNone(be.player_view(s, 0)["last_trick"])
        lead = s["turn"]
        s = be.apply_move(s, lead, s["hands"][lead][0])
        s = be.apply_move(s, 1 - lead, s["hands"][1 - lead][0])
        # both players can see the resolved trick
        for viewer in (0, 1):
            self.assertEqual(be.player_view(s, viewer)["last_trick"], s["last_trick"])
